...
def predict_outcome():
    ...
    total_results = Fs.home_wins + Fs.away_wins + Fs.draw

    Fs.home_win_percentage = round((Fs.home_wins / total_results) * 100, 1)
    Fs.away_win_percentage = round((Fs.away_wins / total_results) * 100, 1)
    Fs.draw_percentage = round((Fs.draw / total_results) * 100, 1)

    calc_avg_goals()

    msg_lab.config(bg='powderblue', text='Matches: '+str(total_results)+'  -  Home Wins: ('+
                   str(Fs.home_wins)+') '+str(Fs.home_win_percentage)+'%'+
                   '  -  Away Wins: ('+str(Fs.away_wins)+') '+
                   str(Fs.away_win_percentage)+'%'+
                   '  -  Draws: ('+str(Fs.draw)+') '+
                   str(Fs.draw_percentage)+'%')

    msg_lab2.config(bg='powderblue', text='Avg Home Goals: '+str(Fs.home_avg_goals)+
                   ' - Avg Away Goals: '+str(Fs.away_avg_goals))

    predicted_hg = round(Fs.home_avg_goals)
    predicted_ag = round(Fs.away_avg_goals)

    msg_lab3.config(bg='powderblue',
                    text='Based soley on these results, the next predicted outcome'
                    ' of this fixture is: '
                    +str(predicted_hg) + " - " + str(predicted_ag))

    Fs.all_text = 'Matches:'+str(total_results)+' - '+  \
    ...
    'Avg Away Goals:'+str(Fs.away_avg_goals)+'\n\n'  \
    'Based soley on these results, the next predicted outcome '  \
    'of this fixture is: '+str(predicted_hg) + " - " + str(predicted_ag)
...
def save_as_csv():
    # create time stamp for first part of unique filename
...
logo_image = Image.open(r'data/logo.png')
...
home_team_combo.current(43)
away_team_combo.current(46)
...
By Bluejam - Self-photographed, CC BY-SA 4.0, https://commons.wikimedia.org/w/index.php?curid=78153606
"""
